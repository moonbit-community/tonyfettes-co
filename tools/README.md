...
A small MoonBit program (its own module, ignored by the root build) that owns
the whole pipeline: it spawns `clang` / `llvm-objcopy` through
`moonbitlang/async`'s `@process`, reads/writes files through `@fs`, and joins
paths with `moonbitlang/x/path`. `main` is an `async fn`; there is no shell
script and no C shim.
